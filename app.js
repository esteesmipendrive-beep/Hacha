// --- HACHA V15.0 | MOTOR DE JUEGO (app.js) ---
...
class HachaEngine {
    ...
    showJudgeButtonsR3() { document.getElementById('r3-q-answer').classList.remove('blur-mode'); document.getElementById('btn-reveal-judge-r3').style.display='none'; document.getElementById('r3-judge-buttons').style.display='flex'; }
    ...
    // --- R4 ---
    startRound4Intro() { this.r4Finalists.forEach(id => { if(this.players[id]) this.players[id].wildcards = 1; }); this.r4TurnIdx = 0; this.broadcast({ type: 'phase', phase: 4 }); this.sync(); this.show('screen-r4-intro'); }
    ...
    startR4Timer() { 
        document.getElementById('btn-r4-timer').style.display = 'none'; document.getElementById('r4-timer-display').style.display = 'block'; document.getElementById('r4-controls-judge').style.display = 'block'; document.getElementById('btn-r4-reveal').style.display = 'block'; document.getElementById('r4-judge-buttons').style.display = 'none';
        ...
        document.getElementById('r4-timer-display').style.color = "var(--danger)"; let t = 15; document.getElementById('r4-timer-display').innerText = t; 
    ...
    toggleAnswerR4() { if(this.r4Interval) clearInterval(this.r4Interval); this.r4WildcardActive = false; this.broadcast({type:'r4-wildcard-end'}); document.getElementById('r4-q-answer').classList.remove('blur-mode'); document.getElementById('btn-r4-reveal').style.display = 'none'; document.getElementById('r4-judge-buttons').style.display = 'flex'; document.getElementById('btn-r4-wildcard').style.display = 'none'; }
    judgeR4(ok) { const activeId = this.r4Finalists[this.r4TurnIdx]; const p = this.players[activeId]; if (ok) { p.quesitos.push(this.r4CurrentCat); AudioEngine.correct(); confetti({ origin: { y: 0.8 } }); if(p.quesitos.length >= 6) return this.triggerVictory(p.name, p.avatar); this.sync(); this.updateR4HostUI(); this.broadcastR4State(); } else { AudioEngine.wrong(); document.getElementById('r4-controls-judge').style.display = 'none'; document.getElementById('r4-controls-rebound').style.display = 'block'; this.broadcastR4ReboundState(); } }
